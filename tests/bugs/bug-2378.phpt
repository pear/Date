...
foreach ($dates as $hs_date) {
    ...
    $ts = $date->getTime();
    echo 'Greenwich       = ' . str_pad($ts, 10) . ' - ' . $date->format2('YYYY-MM-DD HH:MI:SSTZH:TZM') . "\n";

    if (PEAR::isError($res = $date->convertTZbyID('Europe/London'))) { print_r($res); exit(); }
    $ts = $date->getTime();
    echo 'London  ' . $date->format2('("UTC"NPTZH)') . " = " . str_pad($ts, 10) . ' - ' . $date->format2('YYYY-MM-DD HH:MI:SSTZH:TZM') . "\n";

    if (PEAR::isError($res = $date->convertTZbyID('Europe/Paris'))) { print_r($res); exit(); }
    $ts = $date->getTime();
    echo 'Paris   ' . $date->format2('("UTC"NPTZH)') . " = " . str_pad($ts, 10) . ' - ' . $date->format2('YYYY-MM-DD HH:MI:SSTZH:TZM') . "\n";

    if (PEAR::isError($res = $date->convertTZbyID('Asia/Jakarta'))) { print_r($res); exit(); }
    $ts = $date->getTime();
    echo 'Jakarta ' . $date->format2('("UTC"NPTZH)') . " = " . str_pad($ts, 10) . ' - ' . $date->format2('YYYY-MM-DD HH:MI:SSTZH:TZM') . "\n";
}
?>
--EXPECT--
